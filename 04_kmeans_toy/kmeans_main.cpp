...
// Read data from file
unsigned int read_data(FILE* f, double** data_p);
...
int main(int argc, char** argv)
{
    ...
    if (argc > 4) {
        ...
        for(int i=0;i<class_n*DATA_DIM;i++) {
          float tmp = centroids[i];
          fwrite(&tmp, sizeof(float), 1, io_file);
        }
        fclose(io_file);
    }
...
unsigned int read_data(FILE* f, double** data_p)
{
    ...
    *data_p = (double*)malloc(sizeof(double) * DATA_DIM * size);
    float * f_data_p = (float*)malloc(sizeof(float) * DATA_DIM * size);

    r = fread(f_data_p, sizeof(float), DATA_DIM*size, f);
    if (r < DATA_DIM*size) {
